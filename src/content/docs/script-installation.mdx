...
1. `root` / `sudo` - This script is designed to be run as a root user and not as a local user.

2. Executing the script will install the necessary dependencies and configure the build environment.

3. Nothing is built until you provide the `all` or a specific module name as a positional parameter to the script.
...
:::danger
Though this script can run on the host system, it is recommended to use a docker container to avoid conflicts with the host system

Qt is especially problematic and tries to use unwanted system dependencies and can cause build failures.

So please, just use the docker method. There are no performance penalties and it is safer.
:::

:::caution
This build script only supports modern versions of the following distributions:
...
:::note[tags]
You use `debian:latest` or a supported tag
:::
